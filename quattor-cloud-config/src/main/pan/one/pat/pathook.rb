...
class PATHook
    ...
    DEFAULT_AUTHORIZED_PORTS = [22]
    DEFAULT_BASE_PORTS = [22]
    ...
    def initialize(id, params)
        ...
        @authorized_ports = params[:authorized_ports] || DEFAULT_AUTHORIZED_PORTS
        @base_ports = params[:base_ports] || DEFAULT_BASE_PORTS
    ...
    def set_remote_info(vmid)
        ...
        @remote_ports = vm['TEMPLATE/INBOUND_PORTS']
    end

    # Set remote ports to translate.
    def set_translate_ports()
        @translate_ports = @base_ports
        if @remote_ports
            @translate_ports += @remote_ports.split(',').map {|x| Integer(x)}
        end
        @translate_ports.uniq!
    end
    ...
    # Tell if the port is authorized to be (un)translated.
    #
    # @param [Integer] port to check
    def authorized?(port)
        @authorized_ports.include? port
    end

    # Add a port translation for each remote ports.
    def add_port_translations()
        @translate_ports.each do |rport|
            ...
            if not self.authorized? rport
                log "unauthorized remote port #{rport} (skip)"
                next
            end
...
The parameter INBOUND_PORTS in the VM template is used to known which ports to
translate in addition of those given with --base-ports option. It must be a
comma-separated list of ports.
"""

            opts.separator ""
            opts.separator "Port translation options"
            opts.on("-a", "--authorized-ports X,Y,Z", Array, "Authorized remote ports to translate (default: #{DEFAULT_AUTHORIZED_PORTS.join(',')})") do |list|
                options[:authorized_ports] = list.map {|x| Integer(x)}
            end
            opts.on("-b", "--base-ports X,Y,Z", Array, "Base remote ports to translate (default: #{DEFAULT_BASE_PORTS.join(',')})") do |list|
                options[:base_ports] = list.map {|x| Integer(x)}
            end
